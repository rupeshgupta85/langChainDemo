...
# Page configuration
st.set_page_config(
    page_title="Legal Document Streamlit RAG Assignment",
    page_icon="💬",
    layout="wide"
)
...
def find_document_file():
    ...
    possible_paths = [
        ...
        os.path.join(script_dir, "Legal_Document_Analysis_Data.txt"),
        # In legaldocument_streamlit_rag_assignment subfolder (one level)
        os.path.join(script_dir, "legaldocument_streamlit_rag_assignment", "Legal_Document_Analysis_Data.txt"),
        # In legaldocument_streamlit_rag_assignment subfolder (one level)
        os.path.join(script_dir, "legaldocument_streamlit_rag_assignment", "Legal_Document_Analysis_Data.txt"),
        # Parent directory
        os.path.join(os.path.dirname(script_dir), "Legal_Document_Analysis_Data.txt"),
        # Parent directory with subfolder
        os.path.join(os.path.dirname(script_dir), "legaldocument_streamlit_rag_assignment", "Legal_Document_Analysis_Data.txt"),
        # Current working directory
        os.path.join(os.getcwd(), "Legal_Document_Analysis_Data.txt"),
        # Relative paths
        "Legal_Document_Analysis_Data.txt",
        "legaldocument_streamlit_rag_assignment/Legal_Document_Analysis_Data.txt",
        "legaldocument_streamlit_rag_assignment/Legal_Document_Analysis_Data.txt",
    ]
    ...
    if _content:
        ...
        if file_path:
            ...
            raise FileNotFoundError(
                "Could not find Legal_Document_Analysis_Data.txt. Please upload a document using the sidebar."
            )
...
uploaded_file = st.sidebar.file_uploader(
    ...
    type=["txt"],
    help="If Legal_Document_Analysis_Data.txt is not found, you can upload a document here"
)
...
try:
    ...
    **Please do one of the following:**
    ...
    2. Place `Legal_Document_Analysis_Data.txt` in one of these locations:
       - Same folder as the script
       - `legaldocument_streamlit_rag_assignment/` subfolder
       - `legaldocument_streamlit_rag_assignment/` subfolder
    """)
    st.stop()
except Exception as e:
...
# Streamlit UI
st.title("💬 Legal Document RAG Streamlit Chat")
...
with st.sidebar:
    ...
    with st.expander("ℹ️ How to Use"):
        st.markdown("""
        1. Upload a document or ensure Legal_Document_Analysis_Data.txt exists
